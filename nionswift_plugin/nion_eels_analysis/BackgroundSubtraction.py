...
def use_interval_as_signal(api, window):
    ...
    if target_data_item and target_interval:
        ...
        target_data_item._data_item.append_data_item(background._data_item)
        target_data_item._data_item.append_data_item(signal._data_item)
        target_display._display.legend_labels = ["Data", "Background", "Signal"]
    ...
    if target_data_item and target_interval:
        for computation in api.library._document_model.computations:
            if computation.processing_id == "eels.background_subtraction2" and target_interval._graphic in computation._inputs:
                ...
                spectrum_image = api._new_api_object(api.library._document_model.get_source_data_items(target_data_item._data_item)[0])
...
class MenuItemDelegate:
    ...
    def menu_item_execute(self, window):
        use_interval_as_signal(self.__api, window)

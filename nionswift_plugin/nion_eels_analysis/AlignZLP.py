...
def align_zlp_xdata(src_xdata: DataAndMetadata.DataAndMetadata, progress_fn=None) -> typing.Tuple[typing.Optional[DataAndMetadata.DataAndMetadata], typing.Optional[DataAndMetadata.DataAndMetadata]]:
    ...
    if src_xdata.is_datum_1d and (src_xdata.is_sequence or src_xdata.is_collection):
        ...
        pos_data = numpy.zeros(s_shape, dtype=numpy.int16)
        ...
        return (DataAndMetadata.new_data_and_metadata(dst_data, src_xdata.intensity_calibration, dimensional_calibrations, data_descriptor=data_descriptor),
                DataAndMetadata.new_data_and_metadata(pos_data, shift_calibration, dimensional_calibrations[:-d_rank]))

    return None, None


def align_zlp_xdata_subpixel(src_xdata: DataAndMetadata.DataAndMetadata, progress_fn=None, method='com') -> typing.Tuple[typing.Optional[DataAndMetadata.DataAndMetadata], typing.Optional[DataAndMetadata.DataAndMetadata]]:
    # check to make sure it is suitable for this algorithm
    if src_xdata.is_datum_1d and (src_xdata.is_sequence or src_xdata.is_collection):
...
def align_zlp(api, window):
    ...
    if src_data_item:
        ...
        dst_xdata, shift_xdata = align_zlp_xdata(src_data_item.xdata, progress)
        ...
        if dst_xdata:
            ...
            shift_data_item.title = "Shifts (max) " + src_data_item.title
...
def align_zlp_com(api, window):
    ...
    if src_data_item:
        ...
        dst_xdata, shift_xdata = align_zlp_xdata_subpixel(src_data_item.xdata, progress, method='com')

        if dst_xdata:
            # create a new data item in the library and set its title.
            shift_data_item = api.library.create_data_item_from_data_and_metadata(shift_xdata)
            shift_data_item.title = "Shifts (com) " + src_data_item.title
            data_item = api.library.create_data_item_from_data_and_metadata(dst_xdata)
...
def align_zlp_fit(api, window):
    ...
    if src_data_item:
        ...
        dst_xdata, shift_xdata = align_zlp_xdata_subpixel(src_data_item.xdata, progress, method='fit')

        if dst_xdata:
            # create a new data item in the library and set its title.
            shift_data_item = api.library.create_data_item_from_data_and_metadata(shift_xdata)
            shift_data_item.title = "Shifts (peak fit) " + src_data_item.title
            data_item = api.library.create_data_item_from_data_and_metadata(dst_xdata)
